...
window.addEventListener("load", async () => {
  ...
  // ✅ Safe: only attach games after everything is ready
  initGames();
});

/* ========================
   PLAY PANEL (Dino + Quiz)
   ======================== */
function initGames() {
  ...
  function startDino() {
    ...
    const keyJump = (e) => { if (e.code === 'Space') jump(); };
    ...
    function loop() {
      ...
      if (dinoState.y >= H - 50) {
        dinoState.y = H - 50;
        dinoState.vy = 0;
        dinoState.onGround = true;
      }

      // player
      c.fillStyle = '#0b5ed7';
      c.fillRect(dinoState.x, dinoState.y - 20, 26, 20);
      c.fillRect(dinoState.x - 6, dinoState.y - 10, 12, 10);
      ...
      for (let i = dinoState.obst.length - 1; i >= 0; i--) {
        ...
        if (dinoState.x < o.x + o.w && dinoState.x + 26 > o.x &&
            dinoState.y - 20 < o.y && dinoState.y > o.y - o.h) {
          dinoState.dead = true;
        }
        if (o.x + o.w < 0) {
          dinoState.obst.splice(i, 1);
          dinoState.score++;
        }
      }
      ...
      if (dinoState.dead) {
        ...
        c.fillText('Game over — tap to restart', 130, 110);
        canvas.onpointerdown = () => { dinoStop(); startDino(); };
        cancelAnimationFrame(dinoRAF);
        window.removeEventListener('keydown', keyJump);
        return;
      }
  ...
  function renderQuiz(choices, answer) {
    ...
    grid.innerHTML = choices.map(c => renderChoiceHTML(c)).join('');
    status.textContent = '';
    ...
    drop.ondrop = e => {
      ...
      if (name === answer.name) {
        ...
        say?.('Try again');
        setTimeout(() => (drop.textContent = 'Drop here'), 700);
      }
    };
  }
  ...
  function renderChoiceHTML(c) {
    ...
    return `<div class="game-card" draggable="true" data-name="${c.name}">
      ${media}<p>${c.name}</p></div>`;
  }

  function buildAnimalsPool() {
    const animals = (window.themeSets && window.themeSets.animals) || [];
